    ...
    <>
      <nav
        className={`fixed top-0 w-full z-40 transition-all duration-500 ease-out ${
          scrolled
            ? "bg-navy/95 backdrop-blur-lg py-4 shadow-lg shadow-black/10"
            : "py-6"
        }`}
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          <Link href="/" className="flex flex-col group cursor-pointer z-50 relative">
            <span className="font-serif text-2xl font-bold tracking-wide text-white group-hover:text-bronze transition-colors duration-300">
              AG.
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8 lg:gap-10">
            ...
            <Link
              href="/contact"
              className={`px-6 py-2 border rounded-sm text-xs uppercase tracking-widest transition-all duration-300 ${
                isActive("/contact")
                  ? "bg-bronze text-navy border-bronze"
                  : "border-white/20 hover:bg-white hover:text-navy hover:border-white"
              }`}
            >
              Contact
            </Link>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-navy/98 z-30 md:hidden flex flex-col justify-center items-center gap-6 transition-all duration-500 ease-out ${
          menuOpen
        ...
        {navLinks.map((link, i) => (
          <Link
            ...
            className={`text-3xl font-serif transition-all duration-300 ${
              isActive(link.href) ? "text-bronze" : "text-white hover:text-bronze"
            }`}
            style={{
